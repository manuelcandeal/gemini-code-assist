...
export function createThemeSwitcher() {
    ...
    themeList.addEventListener('click', (e) => {
        ...
        if (themeItem) {
            const theme = (themeItem as HTMLElement).dataset.theme;
            if (theme) {
                setTheme(theme);
    ...
    document.addEventListener('click', (e) => {
        if (!themeContainer.contains(e.target as Node)) {
            themeList.classList.add('hidden');
        }
    });
...
export function showScreen(screenId: 'main' | 'userDataEdit' | 'userDataView') {
